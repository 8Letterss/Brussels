    ...
    public  void  leftFactor(){
        ...
        while(true){
            ...
            for(i = 0;i<charList.size(); i++){
                ...
                ArrayList<String> tempProductionAi=new ArrayList<>();//记录公共因子或原产生式
                ArrayList<String> tempProductionAj=new ArrayList<>();//记录非公共因子
                List<String> productionAi = map.get(ai);//获取和ai有关的产生式右部
                ...
                while(ex==1){//找出公共因子
                    flag=0;
                    Map<Character,Integer>m=new HashMap<>();//出现次数
                    int max=0;
                    char max_k='x';
                ...
                for(ii=0;ii<productionAi.size()&&sb.toString().length()!=0;ii++){
                    ...
                    if(current.length()>=tempL){
                        if(current.substring(0,tempL).equals(common)){//存在左公共因子
                            ...
                            if(current.length()>tempL)//判断是否产生e
