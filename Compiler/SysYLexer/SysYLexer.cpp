...
int char_type(char ch);//返回字符ch的类型
...
char dele_annotation[255];//保存处理好注释的一行字符串
char dele_tab[255];//保存处理好退格符的字符串
...
int main(){
        ...
        while(str != NULL){
            ...
            for(int i = 0; i < strlen(str); i++){
                ...
                if(char_type(str[i]) == 5 && !is_annotation){ //分隔符
                    ...
                    printf("ERROR IN LINE %d, ERROR TYPE: 出现词法中未定义字符。\n", row_number);
    ...
    if(is_annotation==true) printf("ERROR, ERROR TYPE: 注释不完整，请检查。\n");
